...
export function BlocExcel () {
  ...
  const formulas = {
    ts: [
      { name: 'INDEX/MATCH', formula: '=INDEX(plage_retour, MATCH(valeur, plage_recherche, 0))', usage: "Remplace VLOOKUP, cherche dans n'importe quelle direction", cas: 'Récupérer un multiple de comparables sur un identifiant unique' },
      { name: 'SUMIFS', formula: '=SUMIFS(sum_range, criteria_range1, criteria1, ...)', usage: 'Somme conditionnelle multi-critères', cas: 'Totaliser des flux par catégorie et par période' },
      { name: 'IFERROR', formula: '=IFERROR(formule, valeur_si_erreur)', usage: 'Éviter les #DIV/0! et #N/A dans les modèles', cas: 'Indispensable dans les tableaux de comparables' },
      { name: 'XLOOKUP', formula: '=XLOOKUP(valeur, plage_rech, plage_retour, [si_absent])', usage: 'INDEX/MATCH simplifié, bidirectionnel (Excel 365)', cas: 'Plus lisible dans les modèles partagés' },
    ],
    pe: [
      { name: 'INDEX/MATCH', formula: '=INDEX(plage_retour, MATCH(valeur, plage_recherche, 0))', usage: "Remplace VLOOKUP, cherche dans n'importe quelle direction", cas: 'Récupérer des données de dette par tranche' },
      { name: 'Data Table', formula: 'Données → Analyse de scénarios → Table de données', usage: 'Matrice de sensitivités automatique', cas: "IRR selon EBITDA entry × exit multiple — attendu en entretien" },
      { name: 'Circularity Switch', formula: '=IF(circ_switch=1, formula_avec_boucle, valeur_approx)', usage: 'Gérer les circularités dans les modèles LBO', cas: 'Intérêts sur dette variable. Activer calcul itératif dans Options Excel.' },
      { name: 'IFERROR', formula: '=IFERROR(formule, valeur_si_erreur)', usage: 'Éviter les erreurs qui cassent tout le modèle', cas: 'Indispensable dans les debt schedules' },
    ],
  };
            ...
            <tbody>
              {activeShortcuts.map((s, i) => (
                <tr key={s.key} className={i % 2 === 0 ? 'bg-blue-50' : 'bg-white'}>
                  <td className="px-4 py-2.5 font-mono text-blue-800 text-xs font-medium">{s.key}</td>
        ...
        <div className="grid md:grid-cols-2 gap-4">
          {formulas[tab].map(f => (
            <div key={f.name} className={`${guideCardClass} p-5`}>
              ...
              <div className="font-mono text-xs bg-slate-900 text-blue-300 rounded-lg px-3 py-2 mb-3 break-all">{f.formula}</div>
        ...
        <ol className="space-y-2">
          {tips[tab].map((tip, i) => (
            <li key={tip} className={`flex gap-4 ${guideCardClass} px-4 py-3`}>
              <div className="flex-shrink-0 w-7 h-7 rounded-lg bg-gradient-to-br from-blue-700 to-indigo-800 text-white text-xs font-serif flex items-center justify-center">{i + 1}</div>
